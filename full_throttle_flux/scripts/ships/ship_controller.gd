...
func _setup_hover_ray() -> void:
	if hover_ray:
		...
		# Detect both ground (Layer 1) and ships (Layer 2)
		# Prevents hovering on top of another ship
		hover_ray.collision_mask = 3  # 1 (ground) + 2 (ships)
...
func _setup_ship_collision_layer() -> void:
	"""Enable ship-to-ship collisions via CharacterBody3D."""
	# Ships are on layer 2, walls on layer 4
	# Mask needs ground (1) + ships (2) + walls (4) = 7
...
func _physics_process(delta: float) -> void:
	# Capture velocity before move_and_slide modifies it
...
func _read_input() -> void:
	# AI sets inputs directly, skip reading from hardware
	if ai_controlled:
		return
...
func respawn(custom_position: Vector3 = Vector3.ZERO, custom_rotation: Basis = Basis.IDENTITY) -> void:
	...
	var source := "last_safe"
	if custom_position != Vector3.ZERO:
		source = "custom"
	elif respawn_manager and respawn_manager.is_initialized:
		source = "respawn_manager"
...
func set_safe_position(position: Vector3, rotation: Basis) -> void:
	"""Manually set the safe respawn position."""
...
func _play_ship_collision_feedback(impact_speed: float) -> void:
	...
	if _collision_shake_enabled and camera and impact_speed > _shake_speed_threshold:
		...
		var final_intensity := _shake_intensity * intensity_ratio
		if camera.has_method("apply_shake"):
			camera.apply_shake(final_intensity)
